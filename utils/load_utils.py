...
def fpv_evs_iterator(scenedir, stride=1, timing=False, dT_ms=None, H=260, W=346, parallel=False, cors=4, tss_gt_us=None):
    ...
    evs_file = glob.glob(osp.join(scenedir, "events.txt"))
    assert len(evs_file) == 1
    # events.txt can be tens of millions of lines; np.loadtxt is ~10x slower
    # than pandas' C parser on files this size.
    import pandas as pd
    evs = pd.read_csv(evs_file[0], sep=" ", comment="#", header=None, dtype=np.float64).values # (N, 4) with [ts_sec, x, y, p]
    evs[:, 0] = evs[:, 0] * 1e6
